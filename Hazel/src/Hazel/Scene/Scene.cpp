...
namespace Hazel
{
	static b2BodyType RigidBodyTypeToBox2DBody(Rigidbody2DComponent::BodyType bodyType)
	{
		...
		HZ_CORE_ASSERT(false, "Unknow body type");
		return b2_staticBody;
	}
	...
	void Scene::OnUpdateRuntime(Timestep ts)
	{
		...
		if (mainCamera)
		{
			...
			auto group_spirte = m_Registry.group<TransformComponent>(entt::get<SpriteRendererComponent>);
			for (auto entity : group_spirte)
			{
				auto [transform, sprite] = group_spirte.get<TransformComponent, SpriteRendererComponent>(entity);
